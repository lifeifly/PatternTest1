...
import com.yele.hu.upgradetools.policy.UpgradeLengthFrame;
...
public class UpgradeLengthPackage {
    ...
    // 升级底层程序
    private UpgradeLengthFrame upgradeFrame;
    ...
    public void startUpdate(int type) {
        ...
        if (upgradeFrame == null) {
            upgradeFrame = new UpgradeLengthFrame(type,path);
        }
        ...
        UpgradeConfig upgradeInfo = new UpgradeConfig(13,upgradeFrame.getPackNum());
        CmdPackage cmdPackage = new CmdPackage(CmdFlag.CMD_UPGRADE_LENGTH_READY, upgradeInfo);
        isUpgrade = true;
        if (listener != null) {
            boolean isSend = listener.sendData(cmdPackage.packCmdStr().getBytes());
            if (!isSend) {
...
//                    sendUpdateData(true);
            ...
            case "UDA":
                ...
                switch (result1){
                    ...
                    case 4:
                        stopTimer();
                        listener.upgradeFailed(0x01,"数据通讯超时");
                        break;
                    case 5:
                        stopTimer();
                        listener.upgradeSuccess();
